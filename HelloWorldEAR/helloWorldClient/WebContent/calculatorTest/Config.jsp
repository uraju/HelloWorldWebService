...
<jsp:useBean id="calculatorTestid" scope="session" class="org.acme.wsdl.CalculatorPortProxy"/>
<%
String submitAction = request.getParameter("submitAction");
if (session.getAttribute("__default_endpoint__") == null) {
    session.setAttribute("__default_endpoint__", calculatorTestid._getDescriptor().getEndpoint());
}
if (submitAction != null && submitAction.equals("Update")) {
    calculatorTestid._getDescriptor().setEndpoint(request.getParameter("endpoint"));
}
else if (submitAction != null && submitAction.equals("Reset")) {
    calculatorTestid._getDescriptor().setEndpoint((String) session.getAttribute("__default_endpoint__"));
}
String endpoint = calculatorTestid._getDescriptor().getEndpoint();

boolean hasAsync = com.ibm.ccl.ws.jaxws.gstc.util.AsyncUtils.hasAsync(calculatorTestid.getClass());
